...
const TemplateSelectionModel = ({isOpen, onClose, onSubmit}: TemplateSelectionModelProps) => {
    ...
    const renderStars = (count: number) => {
        return Array(5).fill(0).map((_, index) => (
            <Star key={index} size={14} className={index < count ? "text-yellow-400" : "text-gray-300"} />
        ))
    }
        ...
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
            {step === "select"? (
                ...
                <DialogTitle className="text-2xl font-bold text-[#E93F3F] flex items-center gap-2">
                    <Plus size={24} className="text-[#E93F3F]"/> Select a Template
                </DialogTitle>
                <DialogDescription>
                    Choose a Template to start a new Playground.
                </DialogDescription>
            </DialogHeader>
            <div className="flex flex-col gap-6 py-4">
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <div className="flex justify-center gap-3 mt-4 pt-4 border-t">
                    <div className="flex items-center text-sm text-muted-foreground">
                        ...
                        <span>
                            ...
                            {selectedTemplate ? "2-5 minutes" : "Select a template to see details"}
                        </span>
                    </div>
                ...
                <DialogHeader>
                  ...
                  <DialogDescription>
                    {templates.find((template) => template.id === selectedTemplate)?.name} Project Configuration
                  </DialogDescription>
                </DialogHeader>
                ...
                <div className="flex flex-col gap-4 py-4">
                  <div className="flex flex-col gap-2">
                    ...
                    <Input id="project-name" placeholder="Enter your project name" value={projectName} onChange={(e) => setProjectName(e.target.value)} />
                  </div>
